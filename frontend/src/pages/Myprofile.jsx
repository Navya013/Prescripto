...
const Myprofile = () => {
  ...
  const updateUserProfileData = async () => {
    try {
      ...
      if (data.success) {
        ...
        // Cache busting to ensure new image shows
        setUserData(prev => ({
          ...prev,
          image: `${prev.image}?t=${Date.now()}`
        }));
  ...
  const handleImageChange = (e) => {
    ...
    if (file) {
      if (image) {
        URL.revokeObjectURL(image); // Prevent memory leak
      }
      setImage(file);
    }
  };
    ...
    <div className='max-w-lg flex flex-col gap-2 text-sm'>
      ...
      {isEdit ? (
        ...
        <img
          ...
          src={`${backendUrl}${userData.image}?t=${new Date().getTime()}`}
          alt="User Profile"
        />
      )}
